import { type RunState, s2WorkflowExecutionStore, type WorkflowEvent } from "@firegrid/tanstack-workflow-s2"
...
export default proof("tanstack-workflow-s2.run-lifecycle")
  .describedAs(
  ...
  .spec(({ property, trialId }) =>
    property("tanstack-workflow-s2.run-lifecycle-proof")
      .s2Lite({ persistence: "local-root" })
      .workload(({ s2Endpoint }) =>
        Effect.gen(function*() {
          if (s2Endpoint === undefined) {
            return yield* new VerificationError({ message: "tanstack-workflow-s2 lifecycle proof requires s2Lite" })
          }

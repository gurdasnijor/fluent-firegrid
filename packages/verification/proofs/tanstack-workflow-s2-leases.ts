...
export default proof("tanstack-workflow-s2.leases")
  .describedAs(
  ...
  .spec(({ property, trialId }) =>
    property("tanstack-workflow-s2.leases-proof")
      .s2Lite({ persistence: "local-root" })
      .workload(({ s2Endpoint }) =>
        Effect.gen(function*() {
          if (s2Endpoint === undefined) {
            return yield* new VerificationError({ message: "tanstack-workflow-s2 lease proof requires s2Lite" })
          }

...
export default proof("tanstack-workflow-s2.timers-signals")
  .describedAs(
  ...
  .spec(({ property, trialId }) =>
    property("tanstack-workflow-s2.timers-signals-proof")
      .s2Lite({ persistence: "local-root" })
      .workload(({ s2Endpoint }) =>
        Effect.gen(function*() {
          if (s2Endpoint === undefined) {
            return yield* new VerificationError({ message: "tanstack-workflow-s2 timer proof requires s2Lite" })
          }

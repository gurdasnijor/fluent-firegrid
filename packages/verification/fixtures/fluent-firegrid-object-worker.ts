...
const runId = "fluent-object:run-1"
const counterKey = "counter-1"
...
const counters = object({
  ...
  handlers: {
    *add(input: { readonly amount: number }) {
      ...
      const recorded = yield* run(() => ({ amount: input.amount, hostId, key }), { name: "record" })
      yield* sleepUntil(5_000)
      const finished = yield* run(() => ({ amount: recorded.amount, hostId, key: recorded.key }), { name: "finish" })
      return {
        amount: finished.amount,
        finishedBy: finished.hostId,
        key: finished.key,
        recordedBy: recorded.hostId
      }
    }
  }
})
...
const host = createS2WorkflowRuntimeHost({
  namespace: `fluent-object-${trialId}`,
  s2Endpoint,
  workflows: bindFluentDefinitions([counters])
})
const binding = createTanStackRuntimeBinding(host, { now: () => 1_000 })
const countersSend = sendObjectClient(binding, counters)(counterKey)
const workflowId = workflowIdForHandler(counters, "add")
...
const server = http.createServer((request, response) => {
  void (async () => {
    ...
    if (request.url === "/send" && request.method === "POST") {
      const reference = await Effect.runPromise(countersSend.add({ amount: 5 }, { runId }))

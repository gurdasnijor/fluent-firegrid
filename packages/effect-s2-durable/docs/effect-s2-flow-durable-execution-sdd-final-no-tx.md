...
**Decision: generate the S2 protocol layer from S2's OpenAPI spec using Effect's OpenAPI generator.** Use the same spec source as upstream S2, but generate the protocol binding in the shape this repository actually wants: Effect `HttpClient`, Effect `Schema`, Effect `Stream`, and typed Effect errors.

- spec source: `s2-streamstore/s2-specs/s2/v1/openapi.json` pinned to the chosen S2 specs commit;
- primary generator: `@effect/openapi-generator` (`openapigen --format httpclient`) for Effect-shaped protocol artifacts;
- comparison/reference generator: HeyAPI (`@hey-api/openapi-ts`), because upstream `s2-sdk-typescript` uses it and it is useful for parity checks against the official TS SDK output;
- handwritten layer: only runtime behaviors OpenAPI cannot describe.

This is not "manual wrapper versus codegen." Codegen is the spine. The correction is that **Effect's OpenAPI generator is the primary generator** because it already emits the native artifacts this repo needs (`Schema`, `Effect`, `Stream`, typed status errors). HeyAPI remains valuable as the upstream-reference path, not as the first implementation target. Do **not** hand-maintain a giant `S2ClientApi` to compensate for either generator.

**Initial spike result (2026-06-23).** `@effect/openapi-generator@4.0.0-beta.78` successfully generates from `s2-specs@329de93f7b240a4daef9edbeb98ced0699aab7d0/s2/v1/openapi.json` with `--format httpclient` once its runtime dependencies are present in the invocation environment (`swagger2openapi`, `json-schema-typed`). The output is an Effect-native protocol module: `Schema` declarations for S2 models, an `S2Protocol` interface, typed `S2ProtocolError<...>` status failures, `readSse` as `Stream.Stream`, and `append`/`read`/`checkTail` as `Effect` programs. It includes the load-bearing REST shapes: `AppendInput.fencing_token`, `AppendInput.match_seq_num`, `AppendAck`, typed 412 append-condition errors, `checkTail`, `read`, and `append`. HeyAPI also successfully generates from the same spec and should remain a parity/comparison check against upstream's official generation path.
...
**Conformance gate L-1.** The spike generates the protocol module from the pinned S2 OpenAPI spec with Effect's OpenAPI generator and compares the generated operation/model coverage against HeyAPI/upstream output. The semantic layer then proves the native SDK behavior flow depends on: `matchSeqNum`, `fencingToken`, append-session ticketing/backpressure, Producer per-record seqnums, serialization roundtrip, dedupe headers, and u64 encode/decode. Until this passes, flow must not add wrapper-specific workarounds.
...
-1. **`effect-s2` fidelity spike — generate first, wrap second.** Pin `s2-streamstore/s2-specs`, run `@effect/openapi-generator` (`openapigen --format httpclient`) into `effect-s2/src/generated`, and keep a HeyAPI generation run as the parity check against upstream's official TS SDK route. The public Effect layer is handwritten only around semantics OpenAPI cannot represent: auth/config layers, normalized S2 domain errors, append-session ticketing/backpressure, Producer, serialization/framing, dedupe headers, u64 codecs, fencing/CAS helpers. Gate: generation is reproducible; generated Effect output includes `append`/`read`/`readSse`/`checkTail`, `Schema` model declarations, and typed append-condition errors; semantic tests prove `matchSeqNum`, `fencingToken`, append-session tickets, Producer per-record seqnums, serialization roundtrip, dedupe headers, and u64 encode/decode. This step explicitly replaces the current broad hand-maintained `S2ClientApi` with generated protocol + narrow semantic capabilities.
...
14. **`effect-s2` generation — Effect OpenAPI generator is the primary route.** Since this repo is Effect-native and the generator emits Effect `HttpClient`, `Schema`, `Stream`, and typed status errors directly, `effect-s2` generates its protocol module with `@effect/openapi-generator`. HeyAPI remains the upstream-parity comparison because S2's official TS SDK uses it. The handwritten layer is limited to semantics OpenAPI cannot encode: scoped configuration, domain error normalization, append-session/Producer lifecycles, serialization/framing, dedupe headers, exact u64 helpers, and ergonomic fenced/CAS append helpers.

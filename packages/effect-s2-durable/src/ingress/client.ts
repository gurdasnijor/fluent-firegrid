...
export type IngressClient<H extends Handlers> = {
  ...
  ) => Effect.Effect<HandlerOutput<H[K]>, DurableFailure>
}
...
export interface DurableIngressClient {
  ...
  objectSendClient<Name extends string, H extends Handlers>(
  ...
  objectAttachClient<Name extends string, H extends Handlers>(
    def: ObjectDefinition<Name, H>,
    key: string
  ): IngressAttachClient<H>
  serviceOutputClient<Name extends string, H extends Handlers>(def: ServiceDefinition<Name, H>): IngressOutputClient<H>
  objectOutputClient<Name extends string, H extends Handlers>(
    def: ObjectDefinition<Name, H>,
    key: string
  ): IngressOutputClient<H>
}
  ...
  Effect.gen(function*() {
    ...
    const attachBy = (def: AnyDef, key: string | undefined, method: string, locator: Locator) =>
      Effect.gen(function*() {
        const codec = codecsOf(def, method)
        const result = yield* invokeEndpoint(
          { object: invocations.objectAttach, service: invocations.serviceAttach },
          def,
          key,
          method,
          locatePayload(locator)
        )
        return yield* Schema.decodeUnknownEffect(codec.output)(result.output).pipe(Effect.mapError(asFailure))
      })
    ...
    const outputBy = (def: AnyDef, key: string | undefined, method: string, locator: Locator) =>
      Effect.gen(function*() {
        const codec = codecsOf(def, method)
        const result = yield* invokeEndpoint(
          { object: invocations.objectOutput, service: invocations.serviceOutput },
          def,
          key,
          method,
          locatePayload(locator)
        ...
        return Option.some(
          yield* Schema.decodeUnknownEffect(codec.output)(result.output).pipe(Effect.mapError(asFailure))
        )
      })
      ...
      (def: AnyDef, key: string | undefined) => (method: string) => (input: unknown, invokeOptions?: InvokeOptions) =>
        Effect.gen(function*() {
          ...
          return {
            ...
            attach: attachBy(def, key, method, locator),
            output: outputBy(def, key, method, locator)
          }
        })
    ...
    return {
      ...
      serviceAttachClient: (def: AnyDef) =>
        materializeClient(def, (method: string) => (locator: Locator) => attachBy(def, undefined, method, locator)),
      objectAttachClient: (def: AnyDef, key: string) =>
        materializeClient(def, (method: string) => (locator: Locator) => attachBy(def, key, method, locator)),
      serviceOutputClient: (def: AnyDef) =>
        materializeClient(def, (method: string) => (locator: Locator) => outputBy(def, undefined, method, locator)),
      objectOutputClient: (def: AnyDef, key: string) =>
        materializeClient(def, (method: string) => (locator: Locator) => outputBy(def, key, method, locator))

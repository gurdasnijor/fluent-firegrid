...
const compileMethod = (
  ...
  const compiledHandler = handler(`${owner}/${method}`, { input: codecs.input, output: codecs.output })(
    program
  ) as Handler<
    unknown,
    unknown,
...
const compileHandlers = (
  ...
  Object.fromEntries(
    Object.entries(handlers).map((
      [method, fn]
    ) => [method, compileMethod(owner, method, fn, codecs[method] as MethodCodecs)])
  )
...
): Record<string, CompiledMethod> => compileHandlers(def.name, def.shared, def.sharedCodecs)

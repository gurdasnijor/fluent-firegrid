...
// the RIGHT surfaces (`objectClient(...)` / `serviceClient(...)`) must keep
// working — and a rejected handler must surface as a TYPED `DurableFailure`,
// not a 500 defect.
...
describe.skipIf(!hasS2())("durable ingress — client-surface guard", () => {
  ...
  it("allows the in-handler serviceClient() surface with a replay-stable child invocation id", async () => {
    const result = await runIngress((ingress) => ingress.serviceClient(ServiceProxy).doubleViaService(6))

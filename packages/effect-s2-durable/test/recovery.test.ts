...
// SLICE A RECOVERY GAP (docs/sdds/effect-s2-durable-consolidation-sdd.md):
// object boot-recovery is intentionally not covered here. The old per-key inbox
// drainer + `ObjectStateDb` that the prior object-recovery tests exercised were
// deleted with the legacy object path. Object recovery (enumerate owner keys +
// restart the pending head over the S2 owner stream, plus object signals) returns
// as a fresh vertical proof in Slice B. The service-recovery tests below are
// unchanged — the service runtime still uses WorkflowDb/RosterDb.

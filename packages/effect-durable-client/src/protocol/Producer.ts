    ...
    const attempt = (
      ...
      Effect.gen(function* () {
        ...
        if (res.status === 200 || res.status === 204) {
          ...
          yield* Ref.update(
            state,
            (s): ProducerState => ({ epoch: s.epoch, lastSeq: nextSeq }),
          )
          if (res.streamClosed) {
        ...
        if (res.status === 403) {
          ...
          return yield* new StaleEpoch({
            currentEpoch: res.producerEpoch ?? current.epoch,
          })
        }
        if (res.status === 409) {
          if (res.streamClosed) {
        ...
        const missing = Http.missingStreamError(
          res.status,
          String(opts.endpoint.url),
        )
...
> =>
  Effect.gen(function* () {
    ...
    const failure = MutableRef.make<Option.Option<AnyProducerFailure>>(
      Option.none(),
    )
    ...
    const sendOne = (
      ...
      Effect.gen(function* () {
        ...
        while (index < subBatches.length) {
          ...
          const result = yield* sendBatch(opts, state, encode, subChunk).pipe(
            Effect.exit,
          )
    ...
    const drainOnce: Effect.Effect<void, never, HttpClient.HttpClient> =
      Effect.gen(function* () {
    ...
    const checkFailure: Effect.Effect<void, AnyProducerFailure> =
      Effect.suspend(() => {
        const f = MutableRef.get(failure)
    ...
    const append = (event: A): Effect.Effect<void, AnyProducerFailure> =>
      Effect.suspend(() => {
        ...
        if (MutableRef.get(closed)) {
          return Effect.fail(
            new TransportError({ cause: new Error("producer closed") }),
          )
        }
    ...
    const epoch: Effect.Effect<number> = Ref.get(state).pipe(
      Effect.map((s) => s.epoch),
    )
    const nextSeq: Effect.Effect<number> = Ref.get(state).pipe(
      Effect.map((s) => s.lastSeq + 1),
    )
    ...
    yield* Effect.addFinalizer(() =>
      flush.pipe(
        Effect.catchAllCause((cause) =>
          Effect.logWarning(
            "effect-durable-client: producer finalizer flush failed",
            cause,
          ),
        ),
      ),

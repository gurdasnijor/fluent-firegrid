 ...
 */
class RetryableHttpStatus extends Data.TaggedError(
  "DurableStream/RetryableHttpStatus",
)<{
...
// 4 retries" means.
/* eslint-disable local/no-fixed-polling -- bounded transport retry backoff, not
 * a durable work-discovery loop. The schedule is capped and stops after four
 * retries. */
    ...
    const attempt = Effect.gen(function* () {
      ...
      yield* emitSpanEvent("retry.attempt", {
        ...
        ...(retryAfterMs === undefined
          ? {}
          : { "durable_streams.retry.retry_after_ms": retryAfterMs }),
      })
...
> =>
  executeWithRetry(
    ...
    (u, h) =>
      applyParams(
        HttpClientRequest.get(u).pipe(HttpClientRequest.setHeaders(h)),
        params,
      ),
    callHeaders,
    extraHeaders,
  ...
  Effect.gen(function* () {
    ...
    if (res.status < 200 || res.status >= 300) {
      return yield* new TransportError({
        cause: new Error(`HEAD ${url}: status ${res.status}`),
      })
    }
    ...
    const result: HeadResult = {
      ...
      ttlSeconds:
        parseInt(headerValue(res, C.STREAM_TTL) ?? "", 10) || undefined,
...
> =>
  Effect.gen(function* () {
    ...
    if (res.status !== 200 && res.status !== 204) {
      return yield* new TransportError({
        cause: new Error(`GET ${url}: status ${res.status}`),
      })
    }

    const nextOffset = (headerValue(res, STREAM_NEXT_OFFSET) ??
      opts.offset) as Offset
...
const postInner = (
  ...
  Effect.gen(function* () {
    ...
    if (opts.producerEpoch !== undefined)
      ...
      (u, h) =>
        HttpClientRequest.post(u).pipe(
          ...
          HttpClientRequest.bodyText(
            opts.body,
            opts.contentType ?? C.CONTENT_TYPE_JSON,
          ),
        ),
      opts.callHeaders,
      extra,
    ...
    const expected = parseInt(
      headerValue(res, C.PRODUCER_EXPECTED_SEQ) ?? "",
      10,
    )
    const received = parseInt(
      headerValue(res, C.PRODUCER_RECEIVED_SEQ) ?? "",
      10,
    )
...
> =>
  Effect.gen(function* () {
    ...
    const res = yield* executeWithRetry(
      endpoint,
      (u, h) => {
        const base = HttpClientRequest.put(u).pipe(
          HttpClientRequest.setHeaders(h),
        )

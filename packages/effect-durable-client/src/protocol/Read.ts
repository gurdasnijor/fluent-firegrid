...
const nextCatchUpState = (res: GetJsonResult): Option.Option<CatchUpState> =>
  ...
  >(
    ...
    (state) => {
      ...
      return Effect.gen(function* () {
        ...
        return [items, nextCatchUpState(res)] as const
      })
    },
  )
...
const longPollUnfold = <T>(
  ...
  Stream.unfoldChunkEffect<LongPollState, T, ReadError, HttpClient.HttpClient>(
    ...
    (state) =>
      Effect.gen(function* () {
        const res = yield* Http.getJson(
          endpoint,
          longPollGetOpts(state, callHeaders),
        )
...
> =>
  Effect.gen(function* () {
    ...
    while (!done) {
      ...
      offset = res.nextOffset
      done = res.upToDate || res.streamClosed
    }
  ...
  >(
    ...
    (state) => {
      ...
      return Effect.gen(function* () {
        ...
        return [Chunk.of(batch), nextCatchUpState(res)] as const
      })
    },
  )

...
const failMissingOrTransport = (
  ...
  return missing !== undefined
    ...
    : Effect.fail(
        new TransportError({
          cause: new Error(`POST returned status ${status}`),
        }),
      )
}
...
> =>
  appendRaw(
    opts.endpoint,
    appendRawOptions(encodeSingleJson(opts.schema, opts.event), opts),
  )
...
> =>
  appendRaw(
    opts.endpoint,
    appendRawOptions(encodeBatchJson(opts.schema, opts.events), opts),
  )
  ...
  Effect.gen(function* () {
    ...
    if (res.status === 403) {
      return yield* new StaleEpoch({
        currentEpoch: res.producerEpoch ?? opts.producerEpoch,
      })
    }
    if (res.status === 409) {
      yield* failIfClosed(res)
...
export const create = (
  ...
  Effect.gen(function* () {
    ...
    if (res.status === 409) {
      return yield* new Conflict({
        reason: "Stream exists with different config",
      })
    }
    return yield* new TransportError({
      cause: new Error(`PUT returned status ${res.status}`),
    })
  })
...
> =>
  Effect.gen(function* () {
    ...
    return yield* new TransportError({
      cause: new Error(`Close returned status ${res.status}`),
    })
  })
...
export const del = (
  ...
  Effect.gen(function* () {
    ...
    if (res.status === 404)
      return yield* new NotFound({ url: String(endpoint.url) })
    return yield* new TransportError({
      cause: new Error(`DELETE returned status ${res.status}`),
    })
  })

...
import type { TestServerHandle } from "./test-server.ts"
...
const runtime = <A, E>(eff: Effect.Effect<A, E, Reqs>) =>
  Effect.runPromise(
    Effect.scoped(
      eff.pipe(Effect.provide(FetchHttpClient.layer)),
  ...
  it("long-poll delivers items appended after the reader starts", async () => {
    ...
    await runtime(
      Effect.gen(function* () {
        ...
        const fiber = yield* s.read({ live: "long-poll" }).pipe(
          Stream.take(3),
  ...
  it("SSE delivers items appended after the reader starts", async () => {
    ...
    await runtime(
      Effect.gen(function* () {
        ...
        const seen = yield* Ref.make<ReadonlyArray<number>>([])
        const fiber = yield* s.read({ live: "sse" }).pipe(
          Stream.take(3),
          Stream.runForEach((msg) =>
            Ref.update(seen, (arr) => [...arr, msg.n]),
          ),
  ...
  it("snapshotThenFollow has no gap under concurrent appends (basic)", async () => {
    ...
    await runtime(
      Effect.gen(function* () {
        ...
        const seen = yield* Ref.make<ReadonlyArray<number>>([])
        const liveFiber = yield* result.live.pipe(
          Stream.take(2),
          Stream.runForEach((msg) =>
            Ref.update(seen, (arr) => [...arr, msg.n]),
          ),
  ...
  it("retries an operation with new headers when onError returns RetryOpts", async () => {
    ...
    await runtime(
      Effect.gen(function* () {
        ...
        const onError = (
          _err: unknown,
        ): Effect.Effect<DurableStream.RetryOpts | undefined> => {
          callCount += 1
          ...
          return Effect.succeed(
            undefined as DurableStream.RetryOpts | undefined,
          )
        }

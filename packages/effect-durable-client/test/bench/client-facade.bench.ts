...
let runtime: ManagedRuntime.ManagedRuntime<
  DurableStreamClient | HttpClient.HttpClient,
  never
>
  ...
  bench("facade", async () => {
    ...
    await runtime.runPromise(
      Effect.gen(function* () {
        ...
        for (let i = 0; i < N_APPEND; i++)
          yield* chat.append(url, { n: i, label: `x${i}` })
      }),
    )
  })
})
...
describe(`producer ${N_PROD} events (define vs facade)`, () => {
  bench("define", async () => {
    const s = DurableStream.define({
      endpoint: { url: server.streamUrl("pr-def") },
      schema: Event,
    })
    await runtime.runPromise(
      Effect.scoped(
    ...
    await runtime.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* (yield* DurableStreamClient).create(url, {
            contentType: "application/json",
          })
          const p = yield* chat.producer(url, {
            producerId: "f",
            lingerMs: 5,
            maxBatchSize: 100000,
          })

...
import { all, execute, run, service, workflow } from "../src/index.ts"
import { bindTestDefinition } from "./bind-test-definition.ts"
import type { ExecutionContext, Operation } from "../src/index.ts"
...
describe("durable-execution-engine-substrate-free", () => {
  it("durable-execution-engine-substrate-free: named journal step is a plain Effect and replays from journal", async () => {
    ...
    const first = await runtimeWith(
      fakeFetch,
      bindTestDefinition(greeter, invocation("greet")).greet("Ada"),
    )
    const replayed = await runtimeWith(
      fakeFetch,
      bindTestDefinition(greeter, invocation("greet")).greet("Ada"),
    )
  ...
  it("durable-execution-engine-substrate-free: generator handlers provide Restate-like authoring affordances", async () => {
    const fakeFetch = makeMemoryDurableStreamsFetch()
    ...
    const hello = await runtimeWith(
      fakeFetch,
      bindTestDefinition(svc, invocation("generator-hello")).hello("Ada"),
    )
    const parallel = await runtimeWith(
      fakeFetch,
      bindTestDefinition(svc, invocation("generator-parallel")).parallel(10),
    )
    const replayed = await runtimeWith(
      fakeFetch,
      bindTestDefinition(svc, invocation("generator-parallel")).parallel(10),
  ...
  it("durable-execution-engine-substrate-free: Effect.all replaces Future combinators without positional counters", async () => {
    const fakeFetch = makeMemoryDurableStreamsFetch()
    const executions: Array<string> = []
    ...
    const first = await runtimeWith(
      fakeFetch,
      bindTestDefinition(calculator, invocation("add-pair")).addPair(10),
    )
    const replayed = await runtimeWith(
      fakeFetch,
      bindTestDefinition(calculator, invocation("add-pair")).addPair(10),
  ...
  it("durable-execution-engine-substrate-free: Effect.fork represents local spawned work without child-session durability", async () => {
    const fakeFetch = makeMemoryDurableStreamsFetch()
    const svc = service({
      name: "forkSvc",
      handlers: {
    ...
    await expect(
      runtimeWith(
        fakeFetch,
        bindTestDefinition(svc, invocation("fork")).nested(10),
      ),
    ).resolves.toBe(12)
  })

  it("durable-execution-engine-substrate-free: schema-encoded failures replay through the typed boundary", async () => {
    const fakeFetch = makeMemoryDurableStreamsFetch()
    const executions = { count: 0 }
    const svc = service({
      name: "failureSvc",
    ...
    await expect(
      runtimeEitherWith(
        fakeFetch,
        bindTestDefinition(svc, invocation("failure")).fail(undefined),
      ),
  ...
  it("durable-execution-engine-substrate-free: local control errors are not written as domain journal rows", async () => {
    const fakeFetch = makeMemoryDurableStreamsFetch()
    const svc = service({
    ...
    await expect(
      runtimeEitherWith(fakeFetch, bindTestDefinition(svc, ctx).fail(undefined)),
    ).resolves.toMatchObject({
      _tag: "Left",
  ...
  it("durable-execution-engine-substrate-free: workflow definitions invoke Effect-native handlers", async () => {
    const fakeFetch = makeMemoryDurableStreamsFetch()
    const patchWorkflow = workflow({
      ...
      handlers: {
        ...
        status: (ctx: ExecutionContext, id: string) =>
          execute(
            ctx,
            run("read-status", Effect.succeed(`status:${id}:modeled`)),
          ),
      },
    })
    ...
    await expect(
      runtimeWith(
        fakeFetch,
        bindTestDefinition(patchWorkflow, invocation("workflow-run")).run(
          "tf-29fy",
        ),
      ),
    ...
    await expect(
      runtimeWith(
        fakeFetch,
        bindTestDefinition(patchWorkflow, invocation("workflow-status")).status(
          "tf-29fy",
        ),
      ),

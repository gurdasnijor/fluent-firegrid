...
export type ImplementHandlers<
  Handlers extends Record<string, HandlerDescriptor>,
> = {
  readonly [Key in keyof Handlers]: (
    input: DescriptorInput<Handlers[Key]>,
  ) => Operation<DescriptorOutput<Handlers[Key]>>
}
...
>(
  ...
  switch (definition._kind) {
    case "service":
      return defineService(implementation) as Definition<
        Name,
        Kind,
        ImplementHandlers<Handlers>
      >
    case "object":
      return defineObject(implementation) as Definition<
        Name,
        Kind,
        ImplementHandlers<Handlers>
      >
    case "workflow":
      return defineWorkflow(implementation) as Definition<
        Name,
        Kind,
        ImplementHandlers<Handlers>
      >

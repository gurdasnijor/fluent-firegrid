...
export type SendRequest<Input = unknown, Output = unknown> = CallRequest<
  Input,
  Output
>
...
export interface DurableExecutionIngress<
  Error = unknown,
  Requirements = never,
> {
...
type ClientMethod<Input, Output, Error, Requirements> = [Input] extends [void]
...
export type Client<
  ...
  Requirements = never,
> = {
  readonly [Key in keyof Handlers]: ClientMethod<
    DescriptorInput<Handlers[Key]>,
    DescriptorOutput<Handlers[Key]>,
    Error,
    Requirements
  >
}
...
> = {
  readonly [Key in keyof Handlers]: ClientMethod<
    DescriptorInput<Handlers[Key]>,
    SendReference<DescriptorOutput<Handlers[Key]>>,
    Error,
    Requirements
  >
}
...
type ClientBinder<Mode extends ClientMode> = <
  ...
  Requirements = never,
>(
  ingress: DurableExecutionIngress<Error, Requirements>,
...
): ReadonlyArray<string> => Object.keys(descriptors)
...
>(
  ...
  ...(key === undefined ? {} : { key }),
  input,
  ...(definition._handlers[handler] === undefined
    ? {}
    : { descriptor: definition._handlers[handler] }),
})
...
const bindIngress =
  (mode: ClientMode) =>
  (
    ingress: DurableExecutionIngress,
    ...
    bindClient(definition, key, (request) => ingress[mode](request))

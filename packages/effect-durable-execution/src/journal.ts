...
export class SessionStream extends Context.Tag(
  "effect-durable-execution/journal/SessionStream",
...
export interface FencedWriterService {
  ...
  readonly append: (
    event: JournalEvent,
  ) => Effect.Effect<void, DurableExecutionError, DurableExecutionRequirements>
}

export class FencedWriter extends Context.Tag(
  "effect-durable-execution/journal/FencedWriter",
)<FencedWriter, FencedWriterService>() {}
...
export interface JournalService {
  readonly step: <A, E, R, Encoded = unknown>(
    ...
    R | DurableExecutionRequirements
  >
  readonly find: (key: string) => Option.Option<JournalEvent>
  readonly append: (
    event: JournalEvent,
  ) => Effect.Effect<void, DurableExecutionError, DurableExecutionRequirements>
...
export class Journal extends Context.Tag("effect-durable-execution/journal")<
  Journal,
  JournalService
>() {}
...
const decodeFailure = <E, Encoded>(
  ...
  options?: RunOptions<E, Encoded>,
): Effect.Effect<never, E | DurableExecutionError> => {
  ...
  > =>
    Effect.gen(function* () {
      ...
      yield* Effect.annotateCurrentSpan({
        "durable_execution.step.key": key,
        "durable_execution.step.replayed": hit === undefined ? "false" : "true",
      })
      if (hit !== undefined && succeededFor(hit, key)) {
        yield* Effect.annotateCurrentSpan(
          "durable_execution.step.served",
          "journal",
        )
      ...
      const result = yield* Effect.either(
        action.pipe(Effect.withSpan("step.action")),
      )

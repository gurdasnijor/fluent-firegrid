// @effect-diagnostics effect/nodeBuiltinImport:off effect/strictEffectProvide:off -- test harness: node:child_process probes for the `s2` binary, and the layer provides are the intentional test wiring.
...
// A real S2 backend for the orchestrator's S2-backed conformance tests: spawn
// `s2 lite` on a free port, seed a basin, and hand back an `S2Client` layer
// pointed at it. This mirrors the harness the `effect-s2-durable` package uses —
// the orchestrator's Layer 1 conformance gate must be validated against real S2
// semantics (match_seq_num CAS, cooperative fencing, tailing reads), not a mock.

const BASIN = "effect-s2-stream-db-test"
...
const writeInitFile = Effect.gen(function*() {
  ...
  const dir = yield* fs.makeTempDirectoryScoped({ prefix: "effect-s2-stream-db-s2lite-" })
...
export const S2LiteLive: Layer.Layer<S2Client> = Layer.unwrap(
  Effect.gen(function*() {
    ...
    const clientLayer = S2Client.layer({
      accessToken: Redacted.make("s2lite-effect-s2-stream-db"),
      basinName: BASIN,
      endpoints: {
        account: endpoint,
        basin: endpoint

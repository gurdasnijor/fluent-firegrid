...
describe("fluent-firegrid-public-surface", () => {
  it("fluent-firegrid-public-surface: definitions expose runtime-bindable metadata", () => {
  ...
  it("fluent-firegrid-public-surface: interface descriptors implement typed definitions", () => {
  ...
  it("fluent-firegrid-public-surface: typed clients derive call and send shape from definitions", async () => {
    const incident = service({
      name: "incident",
      handlers: {
        *triage(input: string): Operation<string> {
    ...
    const ingress: FluentIngress<never> = {
      call: (request) =>
        Effect.sync(() => {
          calls.push(request)
      ...
      send: (request) =>
        Effect.sync(() =>
          sends.push(request),
        ).pipe(Effect.as({ invocationId: `send:${request.name}:${request.handler}` })),
    }

    const callResult = await Effect.runPromise(client(ingress, incident).triage("INC-1"))
    const sendResult = await Effect.runPromise(sendClient(ingress, incident).triage("INC-2"))
  ...
  it("fluent-firegrid-public-surface: root exports do not expose scheduler internals", () => {

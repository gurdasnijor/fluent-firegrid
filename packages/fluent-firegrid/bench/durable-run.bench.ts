...
beforeAll(async () => {
  ...
  for (const size of BENCH_SIZES) {
    ...
    const total = await runScoped(
      runtime,
      execute(invocation(url), durableRunOperation(size, (index) => index)),
    )
...
// fluent-firegrid-keystone.BENCHMARK.2
for (const size of BENCH_SIZES) {
  describe(`fluent-firegrid first execution ${size} runs`, () => {
    bench(
  ...
  describe(`fluent-firegrid replay ${size} runs`, () => {
    bench(

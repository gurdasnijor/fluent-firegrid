...
`@firegrid/log` wraps `@s2-dev/streamstore` in a behavioral Effect service. The
...
```sh
pnpm add effect @s2-dev/streamstore @firegrid/log
```
...
import { AppendRecord, S2Client } from "@firegrid/log"
...
import { publish, readDecoded, S2Client } from "@firegrid/log"
...
import { conditionalAppend } from "@firegrid/log"

import { createWorkflow, defineWorkflowRuntime, every, materializeWorkflowSchedules } from "@firegrid/fluent/runtime"
...
export default proof("store.runtime-schedule-sweep")
  .describedAs(
  ...
  .spec(({ property, trialId }) =>
    property("store.runtime-schedule-sweep-proof")
      .s2Lite({ persistence: "local-root" })
      .workload(({ s2Endpoint }) =>
        Effect.gen(function*() {
          if (s2Endpoint === undefined) {
            return yield* new VerificationError({
              message: "store runtime schedule proof requires s2Lite"
            })
          }

...
const waitForDeposedStarted = (
  ...
  const readStartedCount = Effect.provide(
    Effect.gen(function*() {
      yield* basins.ensure({ basin: "firegrid-objects" })
      const basinApi = yield* basin("firegrid-objects")
      yield* basinApi.streams.ensure({ stream: streamName })
      const stream = yield* s2Stream("firegrid-objects", streamName)
...
export default proof("store.object-live-fencing")
  .describedAs(
  ...
  .spec(({ property, trialId }) => {
    ...
    return property("store.object-live-fencing-proof")
      ...
      .workload(({ s2Endpoint }) =>
        Effect.gen(function*() {
          ...
          const deposedRequest = yield* requestJson(`${hostA}/deposed-add?by=5`, { method: "POST" }).pipe(
            Effect.forkDetach
          )
          ...
          yield* Fiber.interrupt(deposedRequest)

...
export default proof("store.object-stale-owner")
  .describedAs(
  ...
  .spec(({ property, trialId }) => {
    ...
    return property("store.object-stale-owner-proof")
      ...
      .workload(({ faults, s2Endpoint }) =>
        Effect.gen(function*() {
          ...
          const slowRequest = yield* requestJson(`${hostA}/slow-add?by=5`, { method: "POST" }).pipe(
            Effect.forkDetach
          )
          ...
          yield* Fiber.interrupt(slowRequest)

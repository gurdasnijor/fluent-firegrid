...
export default proof("effect-s2.capability-a.atomic-replay")
  .describedAs(
    "Proves atomic append + replay rejection for the @firegrid/log substrate."
  )
...
The example is intentionally ordinary Effect and ordinary `@firegrid/log` usage. The
...
```sh
pnpm --filter @firegrid/proofs proofs
```
...
- Live substrate proofs for `@firegrid/log`: capability A, conditional append,

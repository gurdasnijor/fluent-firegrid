...
import { layer as ChdbLayer } from "@firegrid/trace"
...
const runScopedWithLayer = async <A, E, R, ER>(
...
describe("property", () => {
  it("fails loudly when no trace runtime is installed", () =>
    runScopedWithLayer(
      ...
      Layer.mergeAll(ChdbLayer({}), VerificationRuntime.layer)
    ))
  ...
  it(
    "starts configured s2 lite and injects trial host environment into process hosts",
    () =>
      runScopedWithLayer(
        Effect.gen(function*() {
          const s2Port = 32201
  ...
  it("writes a trial report artifact for passing checks", () =>
    runScopedWithLayer(
      ...
      Layer.mergeAll(LiveTraceLayer, NodeFileSystem.layer)
    ))

...
**Phase 0 / Layer 0 are done enough. Stop hardening generation before runtime work.** The active checkout has a clean `effect-s2` production surface and no longer contains active `effect-s2-stream-db` or `effect-s2-durable` packages. The generated `effect-s2/src/generated` artifacts are contract/drift detection for the S2 protocol; the runtime should build on `effect-s2/src/S2Client.ts` semantic capabilities. Do not reintroduce snapshot generator infrastructure, `src/heyapi-client.ts`, `Channel.ts`, or broad legacy adapter packages just to continue.

**Layer 1 is merged, but its validation boundary needs correction before more runtime surface is added.** `packages/effect-s2-flow` now contains the orchestrator substrate (`ViewOrchestrator`, `OwnedOrchestrator`, `CheckTail`, `FlowError`). Do not redo the concurrency design or create another transport wrapper to compensate for it, but do remove any in-repo fake S2 substrate that the layer currently depends on.

**Next PR: corrective vertical slice 1, `Orchestrator over real effect-s2 stream semantics`.** The next unit of work is not a TableView API expansion. A vertical slice must interact with the durable infrastructure path: `effect-s2/src/S2Client.ts` / `StreamApi` and the S2 semantics documented by the official S2 reference. Delete the fake `StreamStore` / `InMemoryStreamStore` path first, then prove the existing orchestrator substrate against real append/read/checkTail/session behavior.

Scope this PR to the smallest durable-infrastructure slice:

- remove `runtime/StreamStore.ts`, `src/test-support/InMemoryStreamStore.ts`, the `./test-support` package export, and tests that validate against a hand-rolled append/read/checkTail model;
- bind `ViewOrchestrator` and `OwnedOrchestrator` to the production `effect-s2` stream capability at the I/O boundary (`S2Client.stream(...)` / `StreamApi`), including `append`, `readSession`, `checkTail`, append-session/Producer behavior where owned writes need ordered durability, and fencing token propagation for owned writes;
- keep any remaining domain declarations private and minimal; no public `EventStream`, `TableView`, `Processor`, or authoring API should be added until the orchestrator has durable validation signal;
- add validation that exercises real S2 semantics through `effect-s2`, using the official published `s2` CLI binary running `s2 lite` as the local service. `repos/s2` is reference material only; do not build or run a vendored server as the validation target. The gate must prove strong-read via `checkTail`, tail folding from a real read session, owned write read-your-writes after ordered ack/apply, fencing-token propagation/rejection behavior, and no reorder when a foreign seq `N` precedes an owned ack at `N+1`.

Do **not** add an `effect-s2-flow` transport facade such as `stream/S2Stream.ts`. Do **not** add an in-memory S2-lite substitute under `src`, `test-support`, or package exports. `effect-s2/src/S2Client.ts` is the production semantic stream surface. Runtime code should call `S2Client.stream(...)` / `StreamApi` directly at the point where it actually performs I/O. Flow may define domain codecs and domain types; it must not re-export or fake a duplicate append/read/readSession API around `effect-s2`.

Do **not** build `TableView`, `Processor`, atomic commit helpers, ownership/activation, durable primitives, authoring APIs, or Cloudflare/Durable Object integrations in this slice.
...
**Conformance gate L1.** Prove the orchestrators against real S2 stream semantics exposed by `effect-s2`, not against a hand-rolled in-memory store. `ViewOrchestrator` must pass eventual read, strong read (`checkTail` barrier), tail folding from `readSession`, pending-read timeout, and recovery-from-cursor tests while reading real S2 records. `OwnedOrchestrator` must pass read-your-writes after ordered durable ack/apply, own write applied once, ordered pending-own ack handling, fencing-token propagation/rejection, and the foreign-record-before-own-ack case: if a foreign record lands at seq `N` and an own ack arrives at `N+1`, the owner must not reorder state. Add live semantic tests against the official published `s2` CLI binary running `s2 lite` so strong reads are linearizable, eventual reads may lag, and owned writes preserve stream order. The vendored `repos/s2` implementation and tests are reference material only; flow must not create a private S2-lite implementation in package code or exported test support.
...
2. **Corrective vertical slice 1: orchestrators over real S2 semantics.** Remove `StreamStore`, `InMemoryStreamStore`, exported `test-support`, and tests that validate against the fake store. Bind the orchestrators to `effect-s2` `S2Client.stream(...)` / `StreamApi` and add live validation against the official published `s2` CLI binary running `s2 lite`. Gate: View eventual/strong reads over real `readSession`/`checkTail`, recovery from cursor, Owned RYW after ordered durable ack/apply, own write applied once, fencing token rejection/propagation, and no reorder when foreign seq `N` precedes own ack `N+1`. **No `S2Stream.ts`, no duplicate `S2Client` / `StreamApi` facade, no vendored server binary as the validation target, and no in-repo S2-lite runtime or test-support package.**
3. **Vertical slice 2: TableView read model over flow streams.** Introduce only the Layer 2 declarations/codecs needed to implement Layer 3. Build `EventStream` / `EventRecord` / `EventCursor`, minimal `Source` / `Sink` declarations if needed by the public API shape, and `TableView` fold/read APIs over the real-S2-backed `ViewOrchestrator`. Gate: typed encode/decode errors; empty/missing stream folds; `getStrong` reflects a concurrent durable write; `changes` delivers every applied mutation once; cold-start-from-snapshot equals full replay.
4. **Vertical slice 3: Processor commit path over owned state.** Implement `Processor`, typed `ctx.emit`, guarantee layers, and the Layer 5 atomic batch helper together because the checkpoint/output behavior is the validation signal. Gate: `effectivelyOnce` durable output on replay; `atLeastOnce` duplicates; `atMostOnce` skips; handler failure moves no checkpoint; `ctx.emit` to undeclared output is a type error; over-budget atomic groups fail as `BatchTooLarge`; downstream shared output replay is deduped via SDK dedupe headers.
5. **Vertical slice 4: Ownership, activation, and failover.** Implement the Layer 6 lease/fence token, owner claiming/renewal, activation on ingress, idle eviction, and cold rehydration together. Gate: stale fenced writer is rejected and poisons its session; failover resumes from checkpoint; submit to a cold key instantiates owner; idle owner checkpoints and tears down; rehydration latency equals failover latency at equal snapshot cadence.
6. **Vertical slice 5: Durable waits.** Implement TimerService, DeferredRegistry, apply-loop completion, and durable suspend/resume together. Gate: `sleep` survives host kill; `TimerFired` is idempotent; deferred/signal await resumes in-memory and after host restart; torn-down invocation re-instantiates on the resolving fact's submit; exclusive calls queue rather than interleave.
7. **Vertical slice 6: Child calls.** Implement `ChildRequested`, inline dispatch through the Activator, child admission by `childId`, and `ChildCompleted` reply routing. Gate: parent calls child and receives result; replay re-dispatches idempotently until admission/completion; dropped first dispatch is recovered; terminal child failure is typed; fire-and-forget does not park.
8. **Vertical slice 7: CurrentInvocationScope and durable primitives.** Implement the runner-provided `CurrentInvocationScope`, then `run` / `state` / `sleep` / `signal` / `awakeable` / `deferred` plus combinators. Gate: free primitives resolve only inside handlers; `run` actions cannot access the scope; replay returns recorded values; `race` / `select` retake the journaled branch; recoverable cancellation is journaled.
9. **Vertical slice 8: Authoring surface and examples.** Implement `service` / `object` / `workflow` / `client` and the worked examples. Gate: the examples pass, including crash-restart-without-duplicate-output for the saga and a read-after-emit-on-own-state case that exercises RYW.

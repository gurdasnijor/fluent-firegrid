...
**Next PR: corrective vertical slice 1, `Orchestrator over real effect-s2 stream semantics`.** The next unit of work is not a TableView API expansion. A vertical slice must interact with the durable infrastructure path: `effect-s2/src/S2Client.ts` / `StreamApi` and the S2 semantics documented by the vendored `repos/s2` reference. Delete the fake `StreamStore` / `InMemoryStreamStore` path first, then prove the existing orchestrator substrate against real append/read/checkTail/session behavior.
...
- add validation that exercises real S2 semantics through `effect-s2`, using a local S2 service/testcontainer backed by vendored `repos/s2` or the vendored `repos/s2` reference harness as the source of truth. The gate must prove strong-read via `checkTail`, tail folding from a real read session, owned write read-your-writes after ordered ack/apply, fencing-token propagation/rejection behavior, and no reorder when a foreign seq `N` precedes an owned ack at `N+1`.
...
2. **Corrective vertical slice 1: orchestrators over real S2 semantics.** Remove `StreamStore`, `InMemoryStreamStore`, exported `test-support`, and tests that validate against the fake store. Bind the orchestrators to `effect-s2` `S2Client.stream(...)` / `StreamApi` and add live or vendored-reference-backed validation. Gate: View eventual/strong reads over real `readSession`/`checkTail`, recovery from cursor, Owned RYW after ordered durable ack/apply, own write applied once, fencing token rejection/propagation, and no reorder when foreign seq `N` precedes own ack `N+1`. **No `S2Stream.ts`, no duplicate `S2Client` / `StreamApi` facade, and no in-repo S2-lite runtime or test-support package.**

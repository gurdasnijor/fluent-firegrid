...
**Layer 1 is merged.** `packages/effect-s2-flow` now contains the orchestrator substrate (`ViewOrchestrator`, `OwnedOrchestrator`, `CheckTail`, `FlowError`) plus deterministic in-memory S2-like test support. Do not redo L1 or create another transport wrapper to compensate for it.

**Next PR: vertical slice 1, `TableView read model over flow streams`.** The next unit of work intentionally spans the Layer 2 vocabulary and Layer 3 materialization because a standalone stream-vocabulary PR has weak validation signal and is prone to duplicating `effect-s2`.

Scope this PR to the smallest end-to-end read-model slice:

- flow-domain declarations that are needed by the slice (`EventStreamDef`, `EventRecord`, `EventCursor`, and only the `Source` / `Sink` type declarations required by the public API shape);
- local Schema encode/decode helpers for flow domain records;
- `TableView` over `ViewOrchestrator`: fold, `get`, `getStrong`, `refresh`, `entries`, `changes`, and `snapshotAndChanges`;
- deterministic tests proving typed codec failures, empty/missing stream folds, strong read after concurrent write, `changes` delivery, and cold-start-from-snapshot equals full replay.

Do **not** add an `effect-s2-flow` transport facade such as `stream/S2Stream.ts`. `effect-s2/src/S2Client.ts` is the production semantic stream surface. Runtime code should call `S2Client.stream(...)` / `StreamApi` directly at the point where it actually performs I/O. Flow may define domain codecs and domain types; it must not re-export a duplicate append/read/readSession API around `effect-s2`.

Do **not** build `Processor`, atomic commit helpers, ownership/activation, durable primitives, authoring APIs, or Cloudflare/Durable Object integrations in this slice.
...
1. **Orchestrator substrate.** Add `runtime/ViewOrchestrator.ts`, `runtime/OwnedOrchestrator.ts`, `runtime/CheckTail.ts`, `runtime/FlowError.ts`, and deterministic in-memory S2-like test support. Implement `ViewOrchestrator` first (apply-on-tail; strong read = `checkTail` + wait), then `OwnedOrchestrator` (fenced writes; pending-own ack ordering; no double-apply; reply after ordered local apply). Bounded queues + drop policy on `changes` + deadline on pending reads. Gate: in-memory model tests prove View eventual/strong reads, recovery from cursor, Owned RYW, own write applied once, and no reorder when foreign seq `N` precedes own ack `N+1`. **Status: merged.**
...
2. **Vertical slice 1: TableView read model over flow streams.** Introduce only the Layer 2 declarations/codecs needed to implement Layer 3. Build `EventStream` / `EventRecord` / `EventCursor`, minimal `Source` / `Sink` declarations if needed by the public API shape, and `TableView` fold/read APIs over `ViewOrchestrator`. Gate: typed encode/decode errors; empty/missing stream folds; `getStrong` reflects a concurrent write; `changes` delivers every applied mutation once; cold-start-from-snapshot equals full replay. **No `S2Stream.ts` or duplicate `S2Client` / `StreamApi` facade.**
3. **Vertical slice 2: Processor commit path over owned state.** Implement `Processor`, typed `ctx.emit`, guarantee layers, and the Layer 5 atomic batch helper together because the checkpoint/output behavior is the validation signal. Gate: `effectivelyOnce` durable output on replay; `atLeastOnce` duplicates; `atMostOnce` skips; handler failure moves no checkpoint; `ctx.emit` to undeclared output is a type error; over-budget atomic groups fail as `BatchTooLarge`; downstream shared output replay is deduped via SDK dedupe headers.
4. **Vertical slice 3: Ownership, activation, and failover.** Implement the Layer 6 lease/fence token, owner claiming/renewal, activation on ingress, idle eviction, and cold rehydration together. Gate: stale fenced writer is rejected and poisons its session; failover resumes from checkpoint; submit to a cold key instantiates owner; idle owner checkpoints and tears down; rehydration latency equals failover latency at equal snapshot cadence.
5. **Vertical slice 4: Durable waits.** Implement TimerService, DeferredRegistry, apply-loop completion, and durable suspend/resume together. Gate: `sleep` survives host kill; `TimerFired` is idempotent; deferred/signal await resumes in-memory and after host restart; torn-down invocation re-instantiates on the resolving fact's submit; exclusive calls queue rather than interleave.
6. **Vertical slice 5: Child calls.** Implement `ChildRequested`, inline dispatch through the Activator, child admission by `childId`, and `ChildCompleted` reply routing. Gate: parent calls child and receives result; replay re-dispatches idempotently until admission/completion; dropped first dispatch is recovered; terminal child failure is typed; fire-and-forget does not park.
7. **Vertical slice 6: CurrentInvocationScope and durable primitives.** Implement the runner-provided `CurrentInvocationScope`, then `run` / `state` / `sleep` / `signal` / `awakeable` / `deferred` plus combinators. Gate: free primitives resolve only inside handlers; `run` actions cannot access the scope; replay returns recorded values; `race` / `select` retake the journaled branch; recoverable cancellation is journaled.
8. **Vertical slice 7: Authoring surface and examples.** Implement `service` / `object` / `workflow` / `client` and the worked examples. Gate: the examples pass, including crash-restart-without-duplicate-output for the saga and a read-after-emit-on-own-state case that exercises RYW.
